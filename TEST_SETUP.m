...
Num_AFT_sym = N;
% noise poser
SNR_dB = 60:5:70;[0 5 10 15 20 25 30];0:5:30;
SNR = 10.^(SNR_dB/10);
...
mean_E = (M_mod==2)+(M_mod~=2)*sqrt((M_mod-1)/6*(2^2));
for tabs_iter = 1:size(taps_set , 1)
    for delay_iter = 1:size(delay_taps_set , 1)
        for doppler_iter = 1:size(Doppler_taps_set , 1)
            ...
            for iesn0 = 0:length(SNR_dB)
                ...
                for ifram = 1:N_fram
                    ...
                    [taps,delay_taps,Doppler_taps,chan_coef] = OTFS_channel_gen(taps_set(tabs_iter), delay_taps_set(delay_iter, :), Doppler_taps_set(doppler_iter, :));
                    N_CP = max(delay_taps);
                    ...
                    if enable_AFT
                        [H_eq_AFT H1 H2]= H_eq_AFT_calc(N_AFT, c0, c1, c2,taps,delay_taps,Doppler_taps,chan_coef);
                        if enable_ML
                            QAM_table = qammod(0:M_mod-1,M_mod,'gray');
                            x_est_AFT = ML_detector(y_AFT,H_eq_AFT,QAM_table, N_AFT);
                            %x_est_AFT = H_eq_AFT'*(H_eq_AFT*H_eq_AFT' +sigma_2_AFT(iesn0)/sig_energy_AFT_sqrt^2*eye(M*N))^(-1)*y_AFT.';
                            %x_est_AFT = H_eq_AFT^(-1)*y_AFT.';
                        else
                    ...
                    if enable_AFT
                        ...
                        data_demapping = qamdemod(x_est_AFT_serial*mean_E, M_mod, 'gray');
                        data_info_est = reshape(de2bi(data_demapping,M_bits),N_bits_perfram,1);
                        errors = sum(xor(data_info_est,data_info_bit));
            ...
            if enable_OTFS
                ...
                hold on
                err_ber_fram_OTFS_total(:, doppler_iter, tabs_iter, delay_iter) = err_ber_fram_OTFS;
            end
            if enable_2d_AFT
                ...
                ylabel('BER'); xlabel('SNR in dB');grid on
                hold on
                err_ber_fram_2d_AFT_total(:, doppler_iter, tabs_iter, delay_iter) = err_ber_fram_2d_AFT;
            end
            if enable_Especial_2d_AFT
                ...
                ylabel('BER'); xlabel('SNR in dB');grid on
                hold on
                err_ber_fram_2d_especial_AFT_total(:, doppler_iter, tabs_iter, delay_iter) = err_ber_fram_2d_especial_AFT;
            end
            if enable_AFT
                err_ber_fram_AFT = err_ber_AFT/N_bits_perfram./N_fram;
                semilogy(SNR_dB, err_ber_fram_AFT,'-*','LineWidth',2);
                legend('1d AFT')
                err_ber_fram_AFT_total(:, doppler_iter, tabs_iter, delay_iter) = err_ber_fram_AFT;
            end
        end
    end
end
